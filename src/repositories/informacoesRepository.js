...
exports.createInformacoes = async (informacoes) => {
    const result = await pool.query(`
        INSERT INTO informacoes (id, foto, nome, cargo, sobre)
        VALUES (1, $1, $2, $3, $4)
        RETURNING *
    `, [informacoes.foto, informacoes.nome, informacoes.cargo, informacoes.sobre]);
    return result.rows[0];
};
...
exports.updateInformacoes = async (informacoes) => {
    const result = await pool.query(`
        UPDATE informacoes
        SET foto = $1, nome = $2, cargo = $3, sobre = $4
        WHERE id = 1
        RETURNING *
    `, [informacoes.foto, informacoes.nome, informacoes.cargo, informacoes.sobre]);
    return result.rows[0];
